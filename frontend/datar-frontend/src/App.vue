    ...
    <a-layout v-else style="min-height: 100vh">
      ...
      <a-layout style="margin-left: 220px">
        <a-layout-header style="background: #001529; padding: 0 24px; display: flex; align-items: center; justify-content: space-between">
          <div style="color: white; font-size: 16px">连接器管理系统</div>
          <div style="display: flex; align-items: center; gap: 16px">
            <span style="color: #ccc; font-size: 14px">
              {{ userInfo.realName || userInfo.username }}
            </span>
            <a-button type="link" style="color: #fff" @click="handleLogout">退出</a-button>
          </div>
        </a-layout-header>
        <!-- 访问历史 Tab 页签栏 -->
...
const getGroupIcon = (iconName) => iconMap[iconName] || CloudServerOutlined
...
const defaultMenuGroups = [
  ...
  },
  {
    ...
    icon: 'CloudOutlined',
    children: [
      { key: '/dataspace', label: '数据空间', icon: 'CloudOutlined' }
    ]
  },
  {
    key: 'g4',
    ...
    children: [
      ...
      { key: '/role', label: '角色管理', icon: 'ApiOutlined' }
    ]
  }
]

// Datar前端有效的路由路径
const validPaths = ['/datasource', '/catalog', '/product', '/order', '/user', '/role', '/dataspace']
...
const loadMenus = async () => {
  try {
    ...
    if (res.code === 200 && res.data && res.data.length > 0) {
      // 转换为 Ant Design Menu 格式，只保留有对应路由的菜单
      menuGroups.value = res.data
        ...
        .map(group => ({
          ...
          children: (group.children || [])
            .filter(menu => menu.fMenuType === 2 && validPaths.includes(menu.fPath))
            .map(menu => ({
              key: menu.fPath,
        ...
        .filter(group => group.children.length > 0) // 过滤掉没有有效子菜单的分组
    } else {
...
const handleLogout = async () => {
  try {
    ...
    if (stored) {
      const info = JSON.parse(stored)
      if (info.userId) {
        await organizationApi.logout(info.userId)
      }
    }
  } catch (e) {
